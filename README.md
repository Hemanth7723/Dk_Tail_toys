...
- Comments explain intent and non-obvious decisions, not code mechanics, and must stay current.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc for their purpose, parameters, and return values.
- Reusable Astro components document their `Props` interface.
- TypeScript follows the formatting rules enforced by ESLint.
